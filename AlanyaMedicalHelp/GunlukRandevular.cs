...
{
    public partial class GunlukRandevular : UserControl
    {
        private readonly AlanyaMedicalHelpEntities _dbContext;
        public GunlukRandevular()
        {
            InitializeComponent();
            _dbContext = new AlanyaMedicalHelpEntities();
        }

        private void GunlukRandevular_Load(object sender, EventArgs e)
        {
            Listele();
        }
        ...
        public void Listele()
        {
            using (var dbContext = new AlanyaMedicalHelpEntities())
            {
                ...
                var values = dbContext.Appointment.Where(x => DbFunctions.TruncateTime(x.AppointmentDate) == today).Select(x => new
                {
                    x.Id,
                    x.Customer.Name,
                    x.Descriptions,
